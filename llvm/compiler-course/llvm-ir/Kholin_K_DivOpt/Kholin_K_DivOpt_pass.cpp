...
void visitor(Function &F) {
  ...
  for (auto &BB : F) {  // function cycle block//
      ...
      if (auto *BinOp = dyn_cast<BinaryOperator>(&I)) {
        if (BinOp->getOpcode() == Instruction::SDiv ||
          ...
          if (ConstantInt *ConstantDivisor = dyn_cast<ConstantInt>(Divisor)) {
            ...
            if (ConstantDivisor->getValue()
                    .isPowerOf2()) {  // standart function for check degree 2
              errs() << "Processing div instruction: " << *BinOp << "\n";
              errs() << "Divisor is a power of 2: " << *ConstantDivisor << "\n";
              if (BinOp->getOpcode() == Instruction::SDiv &&
                  ConstantDivisor
                      ->isNegative()) {  // except situation when negative
    ...
    for (auto *BinOp :
         DivsToReplace) {  // replacing all div instructions to shift
